        ...
        private void ValidateAccessHandler(object sender, SecurityAccessEventArgs e)
        {
            ...
            switch (e.Action)
            {
                ...
                case ForumAction.PollVote:
                    ...
                    //if (CommunitySecurity.CheckPermissions(new Topic() { ID = question.TopicID}, Constants.PollVoteAction))
                    //{

                    //    if (!question.TopicPoll.Closed)
                    //        _view.IsAccessible = !CheckUserPollAnswer(question);

                    //    else if (question.TopicPoll.Closed && CommunitySecurity.CheckPermissions(question.TopicPoll, Constants.TopicCloseAction))
                    //        _view.IsAccessible = !CheckUserPollAnswer(question);
