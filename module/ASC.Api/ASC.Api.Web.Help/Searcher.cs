                ...
                if (IsIndexingNeeded(lastModified)||!IsOperational)
                {
                    try
                    {
                        ...
                        foreach (var entryPoint in documentation)
                        {
                            ...
                            //Id keys
                            pointDoc.Add(new Field("point", entryPoint.Name, Field.Store.YES, Field.Index.UN_TOKENIZED,
                            ...
                            foreach (var pointMethod in entryPoint.Methods)
                            {
                                ...
                                //Id keys
                                doc.Add(new Field("point", entryPoint.Name, Field.Store.YES, Field.Index.UN_TOKENIZED,
                                                  Field.TermVector.YES));
                                doc.Add(new Field("path", pointMethod.ToString(), Field.Store.YES,
                                                  Field.Index.UN_TOKENIZED, Field.TermVector.YES));

                                doc.Add(new Field("url", pointMethod.Path, Field.Store.YES, Field.Index.UN_TOKENIZED,
                                                  Field.TermVector.NO));
                                if (!string.IsNullOrEmpty(pointMethod.Notes))
                                    doc.Add(new Field("notes", pointMethod.Notes, Field.Store.YES, Field.Index.TOKENIZED,
                                                      Field.TermVector.NO));
