                    ...
                    case TokenValue.Tvevent:
                        ...
                        if (_curPropToken.TokenVal == TokenValue.Tdtstart)
                        {
                            ...
                            if (!isAllDay && !isUTC && _curCalendar.TimeZone != null)
                                _curEvent.UtcStartDate = dateTime.AddMinutes((-1) * (int)_curCalendar.TimeZone.GetUtcOffset(DateTime.UtcNow).TotalMinutes);
                            else
                        ...
                        else if (_curPropToken.TokenVal == TokenValue.Tdtend)
                        {
                            ...
                            if (!isAllDay && !isUTC && _curCalendar.TimeZone != null)
                                _curEvent.UtcEndDate = dateTime.AddMinutes((-1) * (int)_curCalendar.TimeZone.GetUtcOffset(DateTime.UtcNow).TotalMinutes);
                            else if (isAllDay)
            ...
            if ((_curPropToken.TokenVal == TokenValue.Tdtstart || _curPropToken.TokenVal == TokenValue.Tdtend) && _component.Peek().TokenVal == TokenValue.Tvevent)
            {
                switch (key.TokenText.ToLower())
                {
                    case "tzid":
                        ...
                        if (_curPropToken.TokenVal == TokenValue.Tdtstart)
                            _curEvent.UtcStartDate = _curEvent.OriginalStartDate.AddMinutes((-1) * (int)tz.GetUtcOffset(DateTime.UtcNow).TotalMinutes);
                        
                        else if (_curPropToken.TokenVal == TokenValue.Tdtend)
                            _curEvent.UtcEndDate = _curEvent.OriginalEndDate.AddMinutes((-1) * (int)tz.GetUtcOffset(DateTime.UtcNow).TotalMinutes);
