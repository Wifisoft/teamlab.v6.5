...
namespace ASC.Core.Notify.Senders
{
    class AWSSender : SmtpSender
    {
        ...
        private NoticeSendResult SendMessage(NotifyMessage m)
        {
            ...
            {
                ToAddresses = m.To.Split(new[] { '|' }, StringSplitOptions.RemoveEmptyEntries).Select(a => new MailAddress(a).Address).ToList(),
            };
            ...
            var from = string.Format("\"{0}\" <{1}>", MimeHeaderUtils.EncodeMime(new MailAddress(m.From).DisplayName), new MailAddress(m.From).Address);
            var request = new SendEmailRequest(from, dest, new Message(subject, body));
            if (!string.IsNullOrEmpty(m.ReplyTo))
            {
                request.ReplyToAddresses.Add(new MailAddress(m.ReplyTo).Address);
            }
