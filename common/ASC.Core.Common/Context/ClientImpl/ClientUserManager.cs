    ...
    class ClientUserManager : IUserManagerClient, IGroupManagerClient
    {
        ...
        public UserInfo SaveUserInfo(UserInfo u)
        {
            ...
            if (u.Status == EmployeeStatus.Active)
            {
                ...
                if (q.ActiveUsers < GetUsers(EmployeeStatus.Active).Count())
                {
                    throw new TenantQuotaException(string.Format("Exeeds the maximum active users ({0})", q.ActiveUsers));
                }
            }
        ...
        private bool IsUserInGroupInternal(Guid userId, Guid groupId, IDictionary<string, UserGroupRef> refs)
        {
            ...
            if (groupId == Constants.GroupEveryone.ID || groupId == Constants.GroupUser.ID)
            {
                return true;
            }
