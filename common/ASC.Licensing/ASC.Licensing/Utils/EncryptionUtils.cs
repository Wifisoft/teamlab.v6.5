// // --------------------------------------------------------------------------------------------------------------------
// // <copyright company="Ascensio System Limited" file="EncryptionUtils.cs">
// //   
// // </copyright>
// // <summary>
...
{
    /// <summary>
    ///   Provides methods to perform symmetric and asymmetric encryption and decryption of data
    /// </summary>
    internal class EncryptionUtils
    {
        ...
        /// <summary>
        ///   Index of the private key header
        /// </summary>
        private const int magic_priv_idx = 0x08;

        /// <summary>
        ///   Index of the public key header
        /// </summary>
        ...
        /// <summary>
        ///   Block size used in encrypt/decrypt actions
        /// </summary>
        ...
        /// <summary>
        ///   Returns RSAParameters from byte[]. Example to get rsa public key from assembly: byte[] pubkey = Assembly.GetExecutingAssembly().GetName().GetPublicKey(); RSAParameters p = SnkUtil.GetRSAParameters(pubkey);
        /// </summary>
        ...
        public static byte[] AsymmetricEncrypt(Assembly assembly, byte[] dataToEncrypt)
        {
            ...
            RSACryptoServiceProvider cipher = GetPublicKeyFromAssembly(assembly);

            return AsymmetricEncrypt(dataToEncrypt, cipher);
        }

        public static byte[] AsymmetricEncrypt(byte[] dataToEncrypt, RSACryptoServiceProvider cipher)
        {
            if (dataToEncrypt == null) throw new ArgumentNullException("dataToEncrypt");
            //Prepare variables used in encryption
            int blockSize = BLOCK_SIZE, index = 0, bytesLeft = dataToEncrypt.Length, totalBytes = 0;
            ...
            using (var memEncryptedTextbuffer = new MemoryStream(totalBytes))
            {
                ...
                while (bytesLeft > 0)
                {
                    ...
                    block = BlockCopy(dataToEncrypt, index, blockSize);
        ...
        /// <summary>
        ///   Asymmetrically decrypts a given byte array using the keys from a strong named keyfile passed as a byte array
        /// </summary>
        ...
        {
            RSACryptoServiceProvider cipher = GetRSAFromSnkBytes(snkFileContent);
            return DoAsymmetricDecrypt(cipher, encryptedData);
        }

        /// <summary>
        ///   Asymmetrically decrypts a given byte array using the keys taken from the provided strong name key file path
        /// </summary>
        /// <param name="encryptedData"> Data to be decrypted </param>
        ...
        {
            //Prepare the cipher
            RSACryptoServiceProvider cipher = GetRSAFromSnkFile(snkFilePath);
            return DoAsymmetricDecrypt(cipher, encryptedData);
        }
        ...
        public static byte[] SymmetricDecrypt(byte[] itemToDecrypt, byte[] key, byte[] iv)
        {
            ...
            using (var msEncryptedText = new MemoryStream(itemToDecrypt, 0, itemToDecrypt.Length, false))
            {
                ...
                using (
                    var csOut = new CryptoStream(msEncryptedText, cipher.CreateDecryptor(key, iv), CryptoStreamMode.Read)
                    )
                {
                    var decryptedData = new byte[itemToDecrypt.Length];
                    ...
                    var trimmedData = new byte[stringSize];
        ...
        public static byte[] SymmetricEncrypt(byte[] itemToEncrypt, byte[] key, byte[] iv)
        {
            ...
            using (var msEncryptedTextBuffer = new MemoryStream())
            using (
                var csOut = new CryptoStream(msEncryptedTextBuffer, cipher.CreateEncryptor(key, iv),
        ...
        /// <returns> New array containing the copied subset, null if the required size would overshoot the bounds of the array </returns>
        ...
        private static bool CheckRSA1(byte[] pubkey)
        {
            ...
            //                             R     S     A     1
            var check = new byte[] {0x52, 0x53, 0x41, 0x31};
        ...
        private static bool CheckRSA2(byte[] pubkey)
        {
            ...
            //                             R     S     A     2
            var check = new byte[] {0x52, 0x53, 0x41, 0x32};
        ...
        private static byte[] DoAsymmetricDecrypt(RSACryptoServiceProvider cipher, byte[] encryptedData)
        {
            ...
            int blockSize = BLOCK_SIZE*2, index = 0, bytesLeft = encryptedData.Length;
            ...
            using (var memDecryptedTextbuffer = new MemoryStream(bytesLeft/2))
            {
                ...
                while (bytesLeft > 0)
                {
                    ...
                    block = BlockCopy(encryptedData, index, blockSize);
