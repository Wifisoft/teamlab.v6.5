	...
	function _init() {
		if (!_cp || _cp.length < 1) {
			_cp = $("<div id='fc_color_picker'/>").colorPicker({
				...
				showModal:      true,
				colors: {1: [
					"#c87b39", "#f88e14", "#ffb403", "#dddf38", "#9fbb4c", "#6cb123", "#288e31",
					"#4cbb78", "#0797ba", "#1d5f99", "#4c76bb", "#3552d2", "#473388", "#6247b8",
					"#884cbb", "#cb59ba", "#ca3083", "#e24e78", "#bf0036", "#e34603",
					"#000000", "#333333", "#dedede", "#ffffff"]},
				colorSet: 1});
		}
	}

	_this.open = function(iconElem, onColorSelect) {
		_init();
...
<div id="fc_cal_editor">\
	...
	<div class="buttons">\
		<a class="baseLinkButton save-btn" href="#">'+htmlEscape(c.dialogButton_save)+'</a>\
		<a class="grayLinkButton cancel-btn" href="#">'+htmlEscape(c.dialogButton_cancel)+'</a>\
		<a class="grayLinkButton delete-btn" href="#">'+htmlEscape(c.dialogButton_delete)+'</a>\
	</div>\
</div>').replace(/\>\s+\</g, "><");
...
<div id="fc_subscription_dlg">\
	...
	<div class="buttons">\
		<a class="baseLinkButton save-btn" href="#">'+htmlEscape(c.dialogButton_save)+'</a>\
		<a class="grayLinkButton cancel-btn" href="#">'+htmlEscape(c.dialogButton_cancel)+'</a>\
		<a class="grayLinkButton unsubs-btn" href="#">'+htmlEscape(c.subscriptionsDialogButton_unsubscribe)+'</a>\
		<a class="grayLinkButton delete-btn" href="#">'+htmlEscape(c.dialogButton_delete)+'</a>\
	</div>\
</div>').replace(/\>\s+\</g, "><");
...
<div id="fc_subscr_editor">\
	...
	<div class="groups"/>\
	<div class="buttons">\
		<a id="fc_subscr_save" class="baseLinkButton" href="#">'+htmlEscape(c.subscriptionsManageDialogButton_save)+'</a><span class="gap">&nbsp;</span>\
		<a id="fc_subscr_cancel" class="grayLinkButton" href="#">'+htmlEscape(c.subscriptionsManageDialogButton_cancel)+'</a>\
	</div>\
</div>').replace(/\>\s+\</g, "><");
...
<div id="fc_event_editor">\
	...
	<div class="buttons">\
		<a class="edit-btn baseLinkButton" href="#">'+htmlEscape(e.dialogButton_edit)+'</a>\
		<a class="save-btn baseLinkButton" href="#">'+htmlEscape(e.dialogButton_save)+'</a>\
		<a class="close-btn baseLinkButton" href="#">'+htmlEscape(e.dialogButton_close)+'</a>\
		<a class="cancel-btn grayLinkButton" href="#">'+htmlEscape(e.dialogButton_cancel)+'</a>\
		<div class="float-right">\
		<a class="delete-btn grayLinkButton" href="#">'+htmlEscape(e.dialogButton_delete)+'</a>\
		<a class="unsubs-btn grayLinkButton" href="#">'+htmlEscape(e.dialogButton_unsubscribe)+'</a>\
		</div>\
	</div>\
...
<div id="fc_delete_settings">\
	...
	<div class="buttons">\
		<a class="save-btn baseLinkButton" href="#">'+htmlEscape(ds.dialogButton_save)+'</a>\
		<a class="cancel-btn grayLinkButton" href="#">'+htmlEscape(ds.dialogButton_cancel)+'</a>\
	</div>\
</div>').replace(/\>\s+\</g, "><");
...
<div id="fc_ical_stream">\
	...
	<div class="buttons">\
		<a class="cancel-btn baseLinkButton" href="#">'+htmlEscape(ic.dialogButton_close)+'</a>\
	</div>\
</div>').replace(/\>\s+\</g, "><");
	...
	function _renderNewSelectorLabel(elem) {
		var l = $(
				'<span class="new-event">' +
					...
					'<span class="label">' + htmlEscape(options.addNewLabel) + '</span>' +
					'<span class="fc-dropdown">&nbsp;</span>'+
				'</span>')
				.appendTo(elem.find(".fc-header-left"));
		
		// add new event / add new iCal-calendar
		_addSelectorLabel = elem.find(".new-event");
		
		elem.find(".new-event").click(
			function (event) {
...
function CategoryDialog(calendar) {
	...
	(function _create() {
		...
		_getControl(kColorBox).click(function() {fcColorPicker.open(this, _setColor);});
		_getControl(kTextColorBox).click(function() {fcColorPicker.open(this, _setTextColor);});
		//
	...
	function _close(changed) {
		...
		_trigger.call(_this, "onClose", _elem, _source, changed);
	}
...
function CategoriesList(calendar) {
	...
	function _updateCategory(elem, source, changed) {
		if (changed) {
			if (elem) {
	...
	function _updateSubscription(elem, source, changed) {
		if (changed) {
			if (elem) {
				calendar.trigger("editCalendar", _this,
						$.extend({action: kCalendarChangeAction}, source),
	...
	function _renderDatepicker() {
		...
		fcMenus.dpMonthMenu = $("<div id='fc_datepicker_mmenu'/>").popupMenu({
			anchor: "left,top",
			direction: "right,up",
			arrow: "down",
			arrowPosition: "50%",
		...
		$("#fc_datepicker .ui-datepicker-title").live("click", function(e) {
			...
			if (e.target.className == "ui-datepicker-year") {
				if (!_dpYearMenuCaller || _dpYearMenuCaller != e.target) {
					...
					if (!fcMenus.dpYearMenu || fcMenus.dpYearMenu.length < 1) {
						fcMenus.dpYearMenu = $("<div id='fc_datepicker_ymenu'/>");
					} else {
	...
	function _doDDX(save) {
		...
		if (save) {    // ------------- SAVE data -------------
			...
			if (curTodo.completed == true) {
				todo.completed.attr("checked", true);
			} else {
...
function EventEditor(calendar, uiBlocker) {
	...
	function _setSelectedDays(indexArr) {
		...
		for (var i = 0; i < rs.dayNamesShort.length; i++) {
			...
			daysSection += 	"<div class='checkbox-day'>" +
								"<input class='repeat-day-" + i + "' type='checkbox' " + ((isSelected == true) ? "checked" : "") + ">" +
								"<div>" + htmlEscape(rs.dayNamesShort[i]) + "</div>" +
							"</div>";
		}
	...
	function _openSettings()
	{
		...
		if ((repeatRule.Freq == ASC.Api.iCal.Frequency.Never) || (repeatRule.Freq == ASC.Api.iCal.Frequency.Daily)) {
			...
			if ((repeatRule.ByDay.length == 1) && (repeatRule.ByDay[0].Order != 0)) {
				...
				_settings.find(".each-day").attr({checked: "checked"});
			}
			else {
	...
	function _doDDX(saveData) {
		...
		if (saveData) {			// ------------- SAVE data -------------
			...
			if (_eventObj.allDay == true) {
				dlg.editor.allday.attr("checked", true);
	...
	function _updateEvent(isCancel) {
		...
		if(isCancel)
		{
			calendar.trigger("editEvent", _this, $.extend( {action: kEventCancelAction, sourceId: _eventObj.source.objectId}, _eventObj) );
			return;
		}
...
function sliceSegs(events, visEventEnds, start, end) {
	...
	for (i=0; i<len; i++) {
		...
		if ((!event.source || !event.source.isHidden) && eventEnd > start && eventStart < end) {
			if (eventStart < start) {
	...
	function _renderEvents(events, modifiedEventId) {
		...
		for (i = 0; i < segs.length; ++i) {
			...
			for (; start < end; addDays(start, +1)) {
				...
				mt = "_" + monthTitle.replace(/[^a-z0-9_]/gi, "_");
				dt = "_" + dayTitle.replace(/[^a-z0-9_]/gi, "_");
...
function DayEventRenderer() {
	...
	function renderDaySegs(segs, modifiedEventId) {
		...
		if ($.isFunction(t._afterRenderDaySegs)) {t._afterRenderDaySegs(segs);}
	}
				...
				if (t.name == "month") {
					if (elTop + seg.outerHeight > rowTops[seg.row].bottom - infoH) {
						...
						info
								...
								.css("left", (seg.left + Math.round(0.5*(seg.outerWidth - info.width()))) + "px")
