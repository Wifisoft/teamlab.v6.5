...
namespace ASC.Web.Studio.Core
{
    ...
    public class EmailOperationService
    {
        ...
        public string SendEmailActivationInstructions(Guid userID, string email)
        {
            ...
            try
            {
                ...
                if (viewer.IsAdmin())
                {
                    ...
                    {
                        string newCookie = SecurityContext.AuthenticateMe(user.Email, CoreContext.Authentication.GetUserPasswordHash(user.ID));
        ...
        private EmailValidationKeyProvider.ValidationResult CheckValidationKey(string queryString)
        {
            ...
            if (String.IsNullOrEmpty(type) || type.ToLower() != ConfirmType.EmailChange.ToString().ToLower())
            {
            ...
            return EmailValidationKeyProvider.ValidateEmailKey(email + type.ToLower(), key, SetupInfo.ValidEamilKeyInterval);
        }
