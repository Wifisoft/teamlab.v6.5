<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="ImportUsersTemplate.ascx.cs" Inherits="ASC.Web.Studio.UserControls.Users.ImportUsersTemplate" %>
 ...
 <tr class="userItem clearFix"><td class="check"><input type="checkbox" onclick="ImportUsersManager.CheckState();" checked="checked" /></td><td class="name"><div class="clearFix"><div class="firstname">
        ...
        </div></div></td><td class="email">
        <input type="text" class="studioEditableInput email" value="${Email}" maxlength="64" onchange="ImportUsersManager.EraseError(this);" />
        </td><td class="remove {{if ImportUsersManager._mobile}}mob{{/if}}"><div onclick="ImportUsersManager.RemoveItem(this);"></div></td><td class="errors"></td></tr>
 </script>