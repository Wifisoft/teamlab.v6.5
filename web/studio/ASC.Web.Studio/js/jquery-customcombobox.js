  ...
  function renderCombobox (select, options) {
    ...
    for (var i = 0, n = options.length; i < n; i++) {
      ...
      if (option.value != -1 && selectvalue.indexOf('xyu' + option.value + 'xyu') !== -1) {
        isselected = true;
        selectoption = option;
        optionstitle.unshift(selectoption.title);
        optionclassname.unshift(selectoption.classname);
      }
      ...
      html = html.concat([
        '<li',
          ...
          ' title="' + '' + '"',
        '>',
        ismultiple ? '<input id="option-' + id + '" type="checkbox"' + (isselected === true ? ' checked="checked"' : '') + ' /><label for="option-' + id + '">' + option.title + '</label>' : option.title,
        '</li>'
      ]);
    }

    if (defaultoption && optionstitle.length === 0) {
      optionstitle.unshift(defaultoption.title);
    }

    html = [
      '<span class="combobox-title">',
        '<span class="inner-text combobox-title-inner-text">',
          optionstitle.length > 0 ? optionstitle.length === 1 ? optionstitle[0] : optionstitle.join(', ') || '' : '',
        '</span>',
      '</span>',
    ...
    o.className = 'custom-combobox' + (selectoptionInd !== -1 ? ' select-value-' + selectoptionInd : '') + (selectclassname ? ' ' + selectclassname : '') + (optionclassname.length > 0 ? ' ' + optionclassname.join(' ') : '');
    if (optionclassname) {
      o.setAttribute('data-optclass', optionclassname.join(' '));
    }
