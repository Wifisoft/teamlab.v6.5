...
;window.ServiceHelper = (function (factory) {
  ...
  function beforeSend (jqXHR, settings) {
    if (settings && isArray(settings.__requestHeaders) && settings.__requestHeaders.length > 0) {
      var
        requestHeader = null,
        requestHeaders = settings.__requestHeaders,
        requestHeadersInd = requestHeaders.length;
      while (requestHeadersInd--) {
        requestHeader = requestHeaders[requestHeadersInd];
        if (requestHeader.header && requestHeader.value) {
          jqXHR.setRequestHeader(requestHeader.header, requestHeader.value);
        }
      }
    }
  ...
  function checkRequest (req) {
    ...
    if (req.__attcount >= maxRequestAttempts) {
  ...
  function checkResponse (req, jqXHR, textStatus) {
    ...
    if (isInvalidRequest) {
      ...
      if (isUnauthenticationRequest) {
        exec('unauthenticated', this, [req.__params]);
      }
  ...
  function requestSuccess (req, eventname, params, responses, paramses, jqXHR, textStatus) {
    ...
    if (obj === null && responses && req.__singleresponse === false) {
      var args = [req.__params], IsValidData = true;
  ...
  function createRequest (eventname, params, type, url, data, options) {
    ...
    if (options && typeof options === 'object') {
      requestHeaders = options.hasOwnProperty('headers') && typeof options.headers === 'object' ? options.headers : requestHeaders;
    ...
    return {
      ...
      __isme : eventname === 'me',
      __issimple : eventname !== 'authentication' && eventname !== 'settings' && eventname !== 'me' && type === 'get' && typeof url === 'string',
      __urls : [],
      __methods : [],
      __paramses : [],
      __responses : [],
  ...
  function createBatchrequest (eventname, params, type, url, data, options, batchrequests) {
    params = params || {};
  ...
  function sendRequests (reqs) {
    ...
    if (reqsInd === -1) {
      ...
      var batchreq = createBatchrequest(null, null, 'post', 'batch.json', {batch : jQuery.toJSON(batch)}, null, batch);
  ...
  function addRequest (eventname, params, type, url, data, options) {
    ...
    if (isArray(url)) {
      ...
      req = createBatchrequest(eventname, params, 'post', 'batch.json', {batch : jQuery.toJSON(batch)}, options, batch);
    } else {
      req = createRequest(eventname, params, type, url, data, options);
    }
