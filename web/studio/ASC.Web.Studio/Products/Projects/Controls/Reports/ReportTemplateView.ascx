...
<script type="text/javascript" language="javascript">
    ...
    {
        if(jq("#cbxSendOnEmail").is(':checked'))
            unlockTemplate();
        else
            lockTemplate();
    }
...
<div id="report_template_popup" style="display: none;">
    ...
    <ascwc:container id="report_template_container" runat="server">
        ...
        <body>
            ...
            <div>
                <div>
                    <a class="baseLinkButton" onclick="javascript:if(jq('#templateTitle').val()!='') { jq.unblockUI(); ASC.Projects.Reports.EditTemplate(jq('#TemplateId').val(), jq('#templateTitle').val(), jq('#periods option:selected').val(), jq('#periodItems option:selected').val(), jq('#hours option:selected').val(), jq('#cbxSendOnEmail').is(':checked')); } else alert('<%=ReportResource.EmptyTemplateTitle %>');" href="javascript:void(0);">
