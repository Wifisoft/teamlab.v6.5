...
namespace ASC.Web.Community.News.Code.DAO
{
    class DbFeedStorage : IFeedStorage
    {
        ...
        public List<Feed> SearchFeeds(string s)
        {
            ...
            return dbManager
                .ExecuteList(Query("events_feed").Select("Id", "Caption", "Text", "Date").Where(GetWhere(s, FeedType.All, Guid.Empty)).OrderBy("Id", false))
        ...
        {
            return ReplyToTagProvider.Comment("event", feed.Id.ToString(), feedComment != null ? feedComment.Id.ToString() : null);
        }
