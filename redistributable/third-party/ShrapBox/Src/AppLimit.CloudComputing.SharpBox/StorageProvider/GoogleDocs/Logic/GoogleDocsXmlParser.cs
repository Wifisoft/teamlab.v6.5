        ...
        public static IEnumerable<BaseFileEntry> ParseEntriesXml(IStorageProviderSession session, String xml)
        {
            ...
            foreach (var entry in entries)
            {
                ...
                BaseFileEntry fsEntry = kind.Equals("folder")
                                            ? new BaseDirectoryEntry(title, 0, Convert.ToDateTime(updated), session.Service, session)
                                            : new BaseFileEntry(title, 0, Convert.ToDateTime(updated), session.Service, session);
